...
public class PeSTOEvaluationFunction {

    private int pColor(boolean isWhite) {
        return isWhite ? 0 : 1;
    }
    ...
    private final int[][] mgPestoTable = {
        mgPawnTable,
        mgKnightTable,
        mgBishopTable,
        mgRookTable,
    ...
    private final int[][] egPestoTable = {
        egPawnTable,
        egKnightTable,
        egBishopTable,
        egRookTable,
    ...
    private final int[] gamePhaseInc= {0,0,1,1,1,1,2,2,4,4,0,0};
    private final int[][] mgTable = new int[12][64];
    private final int[][] egTable = new int[12][64];
    ...
    public int eval(Spot[][] spots, boolean isWhite){
        int[] mg = {
            0, 0
        };
        int[] eg = {
        ...
        for(int i = 0; i <= 7; i++) {
            for(int j = 0; j <= 7; j++) {
                ...
                if(piece != null) {
                    ...
                    int index = i * 8 + j;
                    mg[pColor(pieceWhite)] += mgTable[position][index];
                    eg[pColor(pieceWhite)] += egTable[position][index];
                    gamePhase += gamePhaseInc[position];
                 }
            }
        }
        int mgScore = mg[pColor(isWhite)] - mg[pColor(!isWhite)];
        int egScore = eg[pColor(isWhite)] - eg[pColor(!isWhite)];
